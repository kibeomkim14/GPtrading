...
class Tree:
    def __init__(self, data=None, left=None, right=None, depth: int = 0):
    ...
    def make_(self, max_depth: int, days: int, grow: bool = False, prev_func: bool = None):
        """
        ...
        if self.depth == 0:
            # At the root, we only assign boolean functions so the outcome is always 1 or 0
            self.f = BOOLEANS[randint(0, len(BOOLEANS))]
        ...
        # After assigning function or terminal, set the node name.
    ...
    def calc_(self, price: float, volume: float):
        """
    ...
    def mutation_(self, days: int, grow: bool = False):
        """
    ...
    def crossover_(self, other, crossPt: int = None):
        """
        ...
        compatible - i.e. if we can't calculate the offspring tree,
